/**
 * C3 Voting Backend - Stores votes in C3 sheet, summary table auto-generated
 */
...
function doPost(e) {
  try {
    ...
    let sheet = ss.getSheetByName(SHEET_C3) || ss.insertSheet(SHEET_C3);
...
function hasVoted(ip) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_C3);
...
function test() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName('C3') || ss.insertSheet('C3');
